...
const ResultsList = ({ items, loading, error, hasLoadedOnce, viewMode }) => (
  <div className={`results-grid ${viewMode}`}>
    ...
    {items.map((document) => (
      <DocumentCard key={document.id ?? document.title} document={document} viewMode={viewMode} />
    ))}
    ...
    {!loading && hasLoadedOnce && items.length === 0 && !error && (
      <div className="document-card document-card--empty">
        <div className="document-card__empty">
          ...
          <h4>No documents match your current filters.</h4>
          <p>Try broadening your search terms or resetting your filters.</p>
        </div>
      </div>
    )}
    ...
    {error && (
      <div className="document-card document-card--error" role="alert">
        <div className="document-card__empty">
          ...
          <h4>We could not load documents.</h4>
          <p>{error}</p>
        </div>
      </div>
    )}
